...
const createBudget = async (req, res) => {
  try {
    ...
    if (!req.user || !req.user.userId) {
      return res
        .status(401)
        .json({ message: "Vous devez être connecté pour ajouter un budget" });
    }
  ...
  try {
    ...
    if (!req.user || !req.user.userId) {
      return res
        .status(401)
        .json({
          message: "Vous devez être connecté pour afficher vos budgets",
        });
    }
...
const getBudgetById = async (req, res) => {
  ...
  if(!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(400).json({ message: "Id de budget invalide" });
  }
  try {
    const budget = await Budget.findById(id);
    if(!budget) {
      res.status(404).json({ message: "Budget non trouvé" });
    }
    res.status(200).json({ message: "Budget trouvé", budget });
  } catch (error) {
    res.status(500).json({ message: "Erreur de serveur" });
    console.log(error);
  }
  ...
  try {
    ...
    if (!req.user || !req.user.userId) {
      return res
        .status(401)
        .json({
          message: "Vous devez être connecté pour modifier votre budget",
        });
    }
    ...
    res
      .status(200)
      .json({ message: "Budget mis à jour avec succès !", updatedBudget });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Erreur de serveur" });
  }
...
const deleteBudget = async (req, res) => {
  try {
    const { budgetId } = req.params;
    // Vérifier si l'ID fourni par user est valid
...
module.exports = { createBudget, getBudgets, updateBudget, deleteBudget, getBudgetById };
